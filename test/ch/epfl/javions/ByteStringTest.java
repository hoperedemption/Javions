...
class ByteStringTest {
    ...
    @Test
    void byteStringBytesInRangeReadsBytesInCorrectOrder1() {
        var bytes = new byte[]{0x12};
        var byteString = new ByteString(bytes);
        assertEquals(0x12, byteString.bytesInRange(0, bytes.length));
    }

    @Test
    void byteStringBytesInRangeReadsBytesInCorrectOrder2() {
        var bytes = new byte[]{0x12, 0x34,(byte) 0xDE};
        var byteString = new ByteString(bytes);
        assertEquals(0x34DEL, byteString.bytesInRange(1, bytes.length));
    }

    @Test
    void byteStringBytesInRangeReadsBytesInCorrectOrder3() {
        var bytes = new byte[]{(byte)0xCD};
        var byteString = new ByteString(bytes);
        assertEquals(0xCDL, byteString.bytesInRange(0, bytes.length));
    }

    @Test
    void byteStringBytesInRangeReadsBytesInCorrectOrder4() {
        var bytes = new byte[]{0x12, 0x34,(byte) 0xDE};
        var byteString = new ByteString(bytes);
        assertEquals(0x1234, byteString.bytesInRange(0, 2));
    }

    @Test
    void byteStringBytesInRangeReadsBytesInCorrectOrder5() {
        var bytes = new byte[]{0x12, 0x34, 0x56, 0x78, (byte) 0x9A, (byte) 0xBC, (byte) 0xDE};
        var byteString = new ByteString(bytes);
        assertEquals(0x56789ABCL, byteString.bytesInRange(2, 6));
    }

    @Test
    void byteStringBytesInRangeReadsBytesInCorrectOrder6() {
        var bytes = new byte[]{0x12, 0x34, 0x56, 0x78, (byte) 0x9A, (byte) 0xBC, (byte) 0xDE, (byte) 0xCA};
        var byteString = new ByteString(bytes);
        assertEquals(0x123456789ABCDECAL, byteString.bytesInRange(0, bytes.length));
    }

    @Test
    void byteStringBytesInRangeReadsBytesInCorrectOrder7() {
        var bytes = new byte[]{0x12, 0x34, 0x56, 0x78, (byte) 0x9A, (byte) 0xBC, (byte) 0xDE};
        var byteString = new ByteString(bytes);
        assertEquals(0x9ABCL, byteString.bytesInRange(4, 6));
    }
    ...
    void byteStringHashCodeDoesStructuralHashing() {
        ...
        int hash1 = bs1.hashCode();
        int hash2 = bs2.hashCode();
